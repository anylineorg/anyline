...
public abstract class AbstractDriverAdapter implements DriverAdapter {
    ...
    public void fillInsertContent(DataRuntime runtime, Run run, Table dest, DataSet<DataRow> set, ConfigStore configs, Boolean placeholder, Boolean unicode, LinkedHashMap<String, Column> columns) {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 void fillInsertContent(DataRuntime runtime, Run run, Table dest, DataSet<DataRow> set, ConfigStore configs, LinkedHashMap<String, Column> columns)", 37));
        }
    }
    ...
    public Run buildUpdateRun(DataRuntime runtime, RunPrepare prepare, DataRow data, ConfigStore configs, Boolean placeholder, Boolean unicode, String ... conditions) {
        Run run = initSelectRun(runtime, prepare);
    ...
    protected long saveObject(DataRuntime runtime, String random, Table dest, Object data, ConfigStore configs, List<String> columns) {
        ...
        if(isNew) {
            ...
            if(null != override) {
                ...
                DataSet<DataRow> exists = selects(runtime, random, prepare, stores);
                if(!exists.isEmpty()) {
                    if(override) {
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, RunPrepare prepare, ConfigStore configs, String ... conditions) {
        ...
        //query拦截
        swt = InterceptorProxy.prepareSelect(runtime, random, prepare, configs, conditions);
        if(swt == ACTION.SWITCH.BREAK) {
            return new DataSet().setTable(table);
        }

        run = buildSelectRun(runtime, prepare, configs, true, true, conditions);
        ...
        if (run.isValid()) {
            if(null == navi || total > 0 || !autoCount) {
                if(null != dmListener) {
                    dmListener.beforeSelect(runtime, random, run, total);
                }
                swt = InterceptorProxy.beforeSelect(runtime, random, run, navi);
                if(swt == ACTION.SWITCH.BREAK) {
                    return new DataSet().setTable(table);
                }
        ...
        if(null != dmListener) {
            dmListener.afterSelect(runtime, random, run, cmd_success, set, System.currentTimeMillis() - fr);
        }
        InterceptorProxy.afterSelect(runtime, random, run, cmd_success, set, navi, System.currentTimeMillis() - fr);
        return set;
    }
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, Procedure procedure, PageNavi navi) {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 DataSet<DataRow> selects(DataRuntime runtime, String random, Procedure procedure, PageNavi navi)", 37));
        }
        return new DataSet();
    }
    ...
    public <T> EntitySet<T> queries(DataRuntime runtime, String random, RunPrepare prepare, Class<T> clazz, ConfigStore configs, String ... conditions) {
        ...
        if (run.isValid()) {
            if(null == navi || !autoCount || total > 0) {
                ...
                fr = System.currentTimeMillis();
                list = query(runtime, random, clazz, run.getTable(), configs, run);
                cmd_success = false;
            }else{
                list = new EntitySet<>();
            }
        ...
        if (null != dmListener) {
            dmListener.afterSelect(runtime, random, run, cmd_success, list, System.currentTimeMillis() - fr);
        }
        InterceptorProxy.afterSelect(runtime, random, run, cmd_success, list, navi, System.currentTimeMillis() - fr);
        return list;
    }
    ...
    public List<Map<String, Object>> maps(DataRuntime runtime, String random, RunPrepare prepare, ConfigStore configs, String ... conditions) {
        ...
        if (run.isValid()) {
            swt = InterceptorProxy.beforeSelect(runtime, random, run, null);
            if(swt == ACTION.SWITCH.BREAK) {
                return new ArrayList<>();
            }
            if (null != dmListener) {
                dmListener.beforeSelect(runtime, random, run, -1);
            }
            if(null != configs) {
        ...
        if (null != dmListener) {
            dmListener.afterSelect(runtime, random, run, cmd_success, maps, System.currentTimeMillis() - fr);
        }
        InterceptorProxy.afterSelect(runtime, random, run, cmd_success, maps, null, System.currentTimeMillis() - fr);
        return maps;
    }
    ...
    public Run buildSelectRun(DataRuntime runtime, RunPrepare prepare, ConfigStore configs, Boolean placeholder, Boolean unicode, String ... conditions) {
        ...
        if(null != joins) {
            for(RunPrepare join:joins) {
                buildSelectRun(runtime, join, new DefaultConfigStore());
            }
        }*/
        ...
        if(run.checkValid()) {
            //构造最终的查询SQL
            run = fillSelectContent(runtime, run, placeholder, unicode);
        }
    ...
    public List<Run> buildSelectSequence(DataRuntime runtime, boolean next, String ... names) {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectSequence(DataRuntime runtime, boolean next, String ... names)", 37));
        }
        return new ArrayList<>();
    }
    ...
    public Run fillSelectContent(DataRuntime runtime, Run run, Boolean placeholder, Boolean unicode) {
        return fillSelectContent(runtime, run.getBuilder(), run, placeholder, unicode);
    }
    ...
    public Run fillSelectContent(DataRuntime runtime, StringBuilder builder, Run run, Boolean placeholder, Boolean unicode) {
        if(null != run) {
            if(run instanceof TableRun) {
                run = fillSelectContent(runtime, builder, (TableRun) run, placeholder, unicode);
            }else if(run instanceof XMLRun) {
                run = fillSelectContent(runtime, builder, (XMLRun) run, placeholder, unicode);
            }else if(run instanceof TextRun) {
                run = fillSelectContent(runtime, builder, (TextRun) run, placeholder, unicode);
            }
    ...
    protected Run fillSelectContent(DataRuntime runtime, StringBuilder builder, TableRun run, Boolean placeholder, Boolean unicode) {
        ...
        fillSelectContentGroup(runtime, builder, run, placeholder, unicode);
        ...
        if(null != unions) {
            for(Run union:unions) {
                ...
                builder.append("\n");
                fillSelectContent(runtime, builder, union, placeholder, unicode);
                run.getRunValues().addAll(union.getRunValues());
            }
        }
        return run;
    }
    protected Run fillSelectContentGroup(DataRuntime runtime, StringBuilder builder, TableRun run, Boolean placeholder, Boolean unicode) {
        RunPrepare prepare = run.getPrepare();
        ConfigStore configs = run.getConfigs();
    ...
    public Run fillMasterTableContent(DataRuntime runtime, StringBuilder builder, TableRun run, RunPrepare prepare) {
        if(prepare instanceof VirtualTablePrepare) {
            ...
            Run fromRun = buildSelectRun(runtime, ((VirtualTablePrepare) prepare).getPrepare(), configs, true, true);
            run.getRunValues().addAll(fromRun.getRunValues());
            String inner = fromRun.getFinalSelect(true);
    ...
    public Run fillJoinTableContent(DataRuntime runtime, StringBuilder builder, TableRun run, RunPrepare prepare) {
        ...
        if(prepare instanceof VirtualTablePrepare) {
            ...
            Run joinRun = buildSelectRun(runtime, ((VirtualTablePrepare) prepare).getPrepare(), new DefaultConfigStore(), true, true);
            run.getRunValues().addAll(joinRun.getRunValues());
            String inner = joinRun.getFinalSelect(true);
            inner = BasicUtil.tab(inner);
    ...
    public Map<String, Object> map(DataRuntime runtime, String random, ConfigStore configs, Run run) {
        ...
        try {
            ...
            if(ConfigStore.IS_THROW_SQL_QUERY_EXCEPTION(configs)) {
                throw new CommandSelectException("查询异常", e);
            }
            if (ConfigStore.IS_LOG_SQL_WHEN_ERROR(configs)) {
                if(ConfigStore.IS_PRINT_EXCEPTION_STACK_TRACE(configs)) {
                    e.printStackTrace();
     ...
     */
    @Override
    public Run buildSelectLengthRun(DataRuntime runtime, String cn, ConfigStore configs) {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现  Run buildSelectLengthRun(DataRuntime runtime, String cn, ConfigStore configs)", 37));
        }
        return null;
    }
    /* *****************************************************************************************************************
    ...
    public <T extends Role> LinkedHashMap<String, T> roles(DataRuntime runtime, String random) throws Exception {
        ...
        try{
            ...
            // 根据系统表查询
            try{
                List<Run> runs = buildSelectRolesRun(runtime);
                if (null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
     ...
     */
    @Override
    public List<Run> buildSelectRolesRun(DataRuntime runtime) throws Exception {
        List<Run> runs = new ArrayList<>();
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectRolesRun(DataRuntime runtime)", 37));
        }
        return runs;
    }
	/* *****************************************************************************************************************
	...
	public String product(DataRuntime runtime, String random) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectProductRun(runtime);
				if (null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Database> List<T> databases(DataRuntime runtime, String random, boolean greedy, Database query) {
		...
		try{
			...
			try{
				...
				List<Run> runs = buildSelectDatabasesRun(runtime, greedy, name);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Database> LinkedHashMap<String, T> databases(DataRuntime runtime, String random, Database query) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectDatabasesRun(runtime, false, name);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectProductRun(DataRuntime runtime) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectProductRun(DataRuntime runtime)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectVersionRun(DataRuntime runtime) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectVersionRun(DataRuntime runtime)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectDatabasesRun(DataRuntime runtime, boolean greedy, Database query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDatabasesRun(DataRuntime runtime, boolean greedy, Database query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public Database database(DataRuntime runtime, int index, boolean create, Database meta, DataSet<DataRow> set) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 Database database(DataRuntime runtime, int index, boolean create, Database meta, DataSet<DataRow> set)", 37));
		}
		return null;
	}
	...
	public String product(DataRuntime runtime, int index, boolean create, String product, DataSet<DataRow> set) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 String product(DataRuntime runtime, int index, boolean create, String product, DataSet<DataRow> set)", 37));
		}
		return null;
	}
	...
	public String version(DataRuntime runtime, int index, boolean create, String version, DataSet<DataRow> set) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 String version(DataRuntime runtime, int index, boolean create, String version, DataSet<DataRow> set)", 37));
		}
		return null;
	}
	...
	public Catalog catalog(DataRuntime runtime, String random) {
		...
		try{
			...
			//根据系统表查询
			try{
				List<Run> runs = buildSelectCatalogRun(runtime, random);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Catalog> LinkedHashMap<String, T> catalogs(DataRuntime runtime, String random, Catalog query) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectCatalogsRun(runtime, false, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Catalog> List<T> catalogs(DataRuntime runtime, String random, boolean greedy, Catalog query) {
		...
		try{
			...
			try{
				...
				List<Run> runs = buildSelectCatalogsRun(runtime, greedy, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectCatalogRun(DataRuntime runtime, String random) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectCatalogRun(DataRuntime runtime, String random)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectCatalogsRun(DataRuntime runtime, boolean greedy, Catalog query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectCatalogsRun(DataRuntime runtime, boolean greedy, Catalog query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public Catalog catalog(DataRuntime runtime, int index, boolean create, Catalog meta, DataSet<DataRow> set) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 Catalog catalog(DataRuntime runtime, int index, boolean create, Catalog meta, DataSet<DataRow> set)", 37));
		}
		return null;
	}
	...
	public Schema schema(DataRuntime runtime, String random) {
		...
		try{
			...
			//根据系统表查询
			try{
				List<Run> runs = buildSelectSchemaRun(runtime, random);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Schema> LinkedHashMap<String, T> schemas(DataRuntime runtime, String random, Schema query) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectSchemasRun(runtime, false, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Schema> List<T> schemas(DataRuntime runtime, String random, boolean greedy, Schema query) {
		...
		try{
			...
			try{
				...
				List<Run> runs = buildSelectSchemasRun(runtime, greedy, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectSchemaRun(DataRuntime runtime, String random) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectSchemaRun(DataRuntime runtime, String random)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectSchemasRun(DataRuntime runtime, boolean greedy, Schema query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectSchemasRun(DataRuntime runtime, boolean greedy, Schema query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public Schema schema(DataRuntime runtime, int index, boolean create, Schema meta, DataSet<DataRow> set) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 Schema schema(DataRuntime runtime, int index, boolean create, Schema meta, DataSet<DataRow> set)", 37));
		}
		return null;
	}
	...
	public <T extends Table<T>> List<T> tables(DataRuntime runtime, String random, boolean greedy, Table<T> query, int types, int struct, ConfigStore configs) {
		...
		try{
			...
			if(null == list || list.isEmpty()) {
                ...
                // 根据系统表查询
                try {
                    List<Run> runs = buildSelectTablesRun(runtime, greedy, query, types, configs);
                    if (null != runs) {
                        int idx = 0;
                        for (Run run : runs) {
                            if (null != navi) {
                                ...
                                if(total <= 0) {
                                    total = count(runtime, random, new DefaultTextPrepare(run.getFinalSelect(false)), configs);
                                    navi.setTotalRow(total);
                                }
                ...
                if (!comment) {
                    try {
                        List<Run> runs = buildSelectTablesCommentRun(runtime, catalog, schema, origin, types);
                        if (null != runs) {
                            int idx = 0;
                            for (Run run : runs) {
                                if (null != navi) {
	...
	protected <T extends Table<T>> void tableMap(DataRuntime runtime, String random, boolean greedy, Table<T> query, ConfigStore configs) {
		...
		//if(null == names || names.isEmpty()) {
			...
			try {
				//缓存 不需要configs条件及分页
				List<Run> runs = buildSelectTablesRun(runtime, greedy, catalog, schema, null, Table.TYPE.NORMAL.value, new DefaultConfigStore());
				if (null != runs && !runs.isEmpty()) {
					int idx = 0;
					for (Run run : runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Table<T>> List<Run> buildSelectTablesRun(DataRuntime runtime, boolean greedy, Table<T> query, int types, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectTablesRun(DataRuntime runtime, Table<T> query, String pattern, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Table<T>> List<Run> buildSelectTablesCommentRun(DataRuntime runtime, Table<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 public <T extends Table<T>> List<Run> buildSelectTablesCommentRun(DataRuntime runtime, Table<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, String random, Table table, boolean init) {
		...
		try {
			long fr = System.currentTimeMillis();
			List<Run> runs = buildSelectDdlRun(runtime, table);
			if (null != runs && !runs.isEmpty()) {
				//直接查询DDL
				int idx = 0;
				for (Run run : runs) {
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, Table table) throws Exception {
		//有支持直接查询DDL的在子类中实现
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, Table table)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, Table table, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, Table table, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public <T extends VertexTable<T>> List<Run> buildSelectVertexesRun(DataRuntime runtime, boolean greedy, VertexTable<T> query, int types, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends VertexTable<T>> List<Run> buildSelectVertexesRun(DataRuntime runtime, VertexTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends VertexTable<T>> List<Run> buildSelectVertexCommentRun(DataRuntime runtime, VertexTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends VertexTable<T>> List<Run> buildSelectVertexCommentRun(DataRuntime runtime, VertexTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends VertexTable<T>> LinkedHashMap<String, T> vertexes(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, VertexTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends VertexTable<T>> LinkedHashMap<String, T> vertexes(DataRuntime runtime, boolean create, LinkedHashMap<String, T> vertexes, VertexTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new LinkedHashMap<>();
		}
	...
	public <T extends VertexTable<T>> List<T> vertexes(DataRuntime runtime, boolean create, List<T> previous, VertexTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends VertexTable<T>> List<T> vertexes(DataRuntime runtime, boolean create, List<T> vertexes, VertexTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new ArrayList<>();
		}
	...
	public List<String> ddl(DataRuntime runtime, String random, VertexTable vertex, boolean init) {
		...
		try {
			long fr = System.currentTimeMillis();
			List<Run> runs = buildSelectDdlRun(runtime, vertex);
			if (null != runs && !runs.isEmpty()) {
				//直接查询DDL
				int idx = 0;
				for (Run run : runs) {
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, VertexTable vertex) throws Exception {
		//有支持直接查询DDL的在子类中实现
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, VertexTable vertex)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, VertexTable vertex, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, VertexTable vertex, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
    ...
    public <T extends VertexTable<T>> T detail(DataRuntime runtime, int index, T meta, VertexTable<T> query, DataRow row) {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends VertexTable<T>> T detail(DataRuntime runtime, int index, T meta, VertexTable<T> query, DataRow row)", 37));
        }
        return meta;
    }
	...
	protected <T extends EdgeTable<T>> void edgeMap(DataRuntime runtime, String random, boolean greedy, EdgeTable<T> query, ConfigStore configs) {
		...
		try {
			//缓存 不需要configs条件及分页
			List<Run> runs =buildSelectEdgesRun(runtime, greedy, catalog, schema, null, EdgeTable.TYPE.NORMAL.value, new DefaultConfigStore());
			if (null != runs && !runs.isEmpty()) {
				int idx = 0;
				for (Run run : runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends EdgeTable<T>> List<Run> buildSelectEdgesRun(DataRuntime runtime, boolean greedy, EdgeTable<T> query, int types, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends EdgeTable<T>> List<Run> buildSelectEdgesRun(DataRuntime runtime, EdgeTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends EdgeTable<T>> List<Run> buildSelectEdgesCommentRun(DataRuntime runtime, EdgeTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends EdgeTable<T>> List<Run> buildSelectEdgesCommentRun(DataRuntime runtime, EdgeTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends EdgeTable<T>> LinkedHashMap<String, T> edges(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, EdgeTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends EdgeTable<T>> LinkedHashMap<String, T> edges(DataRuntime runtime, boolean create, LinkedHashMap<String, T> edges, EdgeTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new LinkedHashMap<>();
		}
	...
	public <T extends EdgeTable<T>> List<T> edges(DataRuntime runtime, boolean create, List<T> previous, EdgeTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends EdgeTable<T>> List<T> edges(DataRuntime runtime, boolean create, List<T> edges, EdgeTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new ArrayList<>();
		}
	...
	public <T extends EdgeTable<T>> T detail(DataRuntime runtime, int index, T meta, EdgeTable<T> query, DataRow row) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends EdgeTable<T>> T detail(DataRuntime runtime, int index, T meta, EdgeTable<T> query, DataRow row)", 37));
		}
		return meta;
	}
	...
	public List<String> ddl(DataRuntime runtime, String random, EdgeTable meta, boolean init) {
		...
		try {
			long fr = System.currentTimeMillis();
			List<Run> runs = buildSelectDdlRun(runtime, meta);
			if (null != runs && !runs.isEmpty()) {
				//直接查询DDL
				int idx = 0;
				for (Run run : runs) {
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, EdgeTable meta) throws Exception {
		//有支持直接查询DDL的在子类中实现
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, EdgeTable meta)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, EdgeTable meta, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, EdgeTable meta, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public <T extends View> List<T> views(DataRuntime runtime, String random, boolean greedy, View query, int types, int struct, ConfigStore configs) {
        ...
        try{
            ...
            if(null == list || list.isEmpty()) {
                ...
                // 根据系统表查询
                try {
                    List<Run> runs = buildSelectViewsRun(runtime, greedy, query, types, configs);
                    if (null != runs) {
                        int idx = 0;
                        for (Run run : runs) {
                            if (null != navi) {
	...
	protected void viewMap(DataRuntime runtime, String random, boolean greedy, View query, ConfigStore configs) {
		...
		try {
			//缓存 不需要configs条件及分页
			List<Run> runs =buildSelectViewsRun(runtime, greedy, catalog, schema, null, View.TYPE.NORMAL.value, new DefaultConfigStore());
			if (null != runs && !runs.isEmpty()) {
				int idx = 0;
				for (Run run : runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectViewsRun(DataRuntime runtime, boolean greedy, View query, int types, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectViewsRun(DataRuntime runtime, View query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectViewsCommentRun(DataRuntime runtime, View query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectViewsCommentRun(DataRuntime runtime, View query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, String random, View view, boolean init) {
		...
		try {
			long fr = System.currentTimeMillis();
			List<Run> runs = buildSelectDdlRun(runtime, view);
			if (null != runs && !runs.isEmpty()) {
				//直接查询DDL
				int idx = 0;
				for (Run run : runs) {
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, View view) throws Exception {
		//有支持直接查询DDL的在子类中实现
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, View view)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, View view, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, View view, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public <T extends MasterTable<T>> List<T> masters(DataRuntime runtime, String random, boolean greedy, MasterTable<T> query, int types, int struct, ConfigStore configs) {
		...
		try{
			...
			if(null == list || list.isEmpty()) {
                ...
                // 根据系统表查询
                try {
                    List<Run> runs = buildSelectMasterTablesRun(runtime, greedy, catalog, schema, origin, types, configs);
                    if (null != runs) {
                        int idx = 0;
                        for (Run run : runs) {
                            if (null != navi) {
                ...
                if (!comment) {
                    try {
                        List<Run> runs = buildSelectMasterTablesCommentRun(runtime, catalog, schema, origin, types);
                        if (null != runs) {
                            int idx = 0;
                            for (Run run : runs) {
                                if (null != navi) {
	...
	protected <T extends MasterTable<T>> void masterMap(DataRuntime runtime, String random, boolean greedy, MasterTable<T> query, ConfigStore configs) {
		...
		try {
			//缓存 不需要configs条件及分页
			List<Run> runs =buildSelectMasterTablesRun(runtime, greedy, catalog, schema, null, MasterTable.TYPE.NORMAL.value, new DefaultConfigStore());
			if (null != runs && !runs.isEmpty()) {
				int idx = 0;
				for (Run run : runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends MasterTable<T>> List<Run> buildSelectMasterTablesRun(DataRuntime runtime, boolean greedy, MasterTable<T> query, int types, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends MasterTable<T>> List<Run> buildSelectMasterTablesRun(DataRuntime runtime, MasterTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends MasterTable<T>> List<Run> buildSelectMasterTablesCommentRun(DataRuntime runtime, MasterTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends MasterTable<T>> List<Run> buildSelectMasterTablesCommentRun(DataRuntime runtime, MasterTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends MasterTable<T>> LinkedHashMap<String, T> masters(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, MasterTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends MasterTable<T>> LinkedHashMap<String, T> masters(DataRuntime runtime, boolean create, LinkedHashMap<String, T> masters, MasterTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new LinkedHashMap<>();
		}
	...
	public <T extends MasterTable<T>> List<T> masters(DataRuntime runtime, boolean create, List<T> previous, MasterTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends MasterTable<T>> List<T> masters(DataRuntime runtime, boolean create, List<T> masters, MasterTable<T> query, int types)", 37));
		}
		if(null == previous) {
            previous = new ArrayList<>();
		}
	...
	public <T extends MasterTable<T>> T detail(DataRuntime runtime, int index, T meta,  MasterTable<T> query, DataRow row) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends MasterTable<T>> T detail(DataRuntime runtime, int index, T meta,  MasterTable<T> query, DataRow row)", 37));
		}
		return meta;
	}
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, MasterTable master) throws Exception {
		//有支持直接查询DDL的在子类中实现
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, MasterTable master)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, MasterTable master, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, MasterTable master, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
    ...
    public Table.Partition partition(DataRuntime runtime, String random, Table table) {
        ...
        try{
            ...
            // 根据系统表查询
            try{
                List<Run> runs = buildSelectTablePartitionRun(runtime, table);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends PartitionTable<T>> LinkedHashMap<String,T> partitions(DataRuntime runtime, String random, boolean greedy, PartitionTable<T> query) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectPartitionTablesRun(runtime, greedy, query);
				if(null != runs) {
					int idx = 0;
					int total = runs.size();
					for(Run run:runs) {
	...
	public <T extends PartitionTable<T>> List<Run> buildSelectPartitionTablesRun(DataRuntime runtime, boolean greedy,  PartitionTable<T> query, int types) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends PartitionTable<T>> List<Run> buildSelectPartitionTablesRun(DataRuntime runtime, boolean greedy,  PartitionTable<T> query, int types)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends PartitionTable<T>> LinkedHashMap<String,T> partitions(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, PartitionTable<T> query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 LinkedHashMap<String, PartitionTable> partitions(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, PartitionTable<T> query)", 37));
		}
		if(null == previous) {
			previous = new LinkedHashMap<>();
		}
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, PartitionTable table) throws Exception {
		List<Run> runs = new ArrayList<>();
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, PartitionTable table)", 37));
		}
		return runs;
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, PartitionTable table, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, PartitionTable table, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public <T extends PartitionTable<T>> T detail(DataRuntime runtime, int index, T meta, PartitionTable<T> query, DataRow row) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends PartitionTable<T>> T detail(DataRuntime runtime, int index, T meta, PartitionTable<T> query, DataRow row)", 37));
		}
		return meta;
	}
	...
	public <T extends Column> LinkedHashMap<String, T> columns(DataRuntime runtime, String random, boolean greedy, Table table, Column query, boolean primary, ConfigStore configs) {
		...
		try {
			...
			// 1.优先根据系统表查询
			try {
				List<Run> runs = buildSelectColumnsRun(runtime,  false, query, configs);
				if (null != runs) {
					int idx = 0;
					for (Run run: runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
			...
			if (null == columns || columns.isEmpty()) {
				try {
					List<Run> runs = buildSelectColumnsRun(runtime, true, query, configs);
					if (null != runs) {
						for (Run run  : runs) {
							String sql = run.getFinalSelect();
	...
	public <T extends Column> List<T> columns(DataRuntime runtime, String random, boolean greedy, Collection<? extends Table> tables, Column query, ConfigStore configs) {
		...
		try {
			...
			List<Run> runs = buildSelectColumnsRun(runtime, false, list, query, configs);
			if (null != runs) {
				int idx = 0;
				for (Run run: runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectColumnsRun(DataRuntime runtime,  boolean metadata, Column query, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectColumnsRun(DataRuntime runtime, Table table, boolean metadata)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectColumnsRun(DataRuntime runtime, boolean metadata, Collection<? extends Table> tables, Column query, ConfigStore configs) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectColumnsRun(DataRuntime runtime, Catalog catalog, Schema schema, Collection<? extends Table> tables, boolean metadata)", 37));
		}
		return new ArrayList<>();
	}
   ...
   public  <T extends Column> LinkedHashMap<String, T> columns(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, Column query) throws Exception {
	...
	public List<Run> buildSelectTagsRun(DataRuntime runtime, boolean greedy, Tag query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectTagsRun(DataRuntime runtime, boolean greedy, Tag query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public PrimaryKey primary(DataRuntime runtime, String random, boolean greedy, PrimaryKey query) {
		...
		try{
			...
			List<Run> runs = buildSelectPrimaryRun(runtime, greedy, query);
			if(null != runs) {
				int idx = 0;
				for(Run run:runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectPrimaryRun(DataRuntime runtime, boolean greedy,  PrimaryKey query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectPrimaryRun(DataRuntime runtime, boolean greedy,  Table table)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends PrimaryKey> T init(DataRuntime runtime, int index, T meta, PrimaryKey query, DataSet<DataRow> set) throws Exception {
        ...
        for(DataRow row:set) {
            ...
            if(BasicUtil.isEmpty(col)) {
                throw new Exception("主键相关列名异常,请检查buildSelectPrimaryRun与primaryMetadataColumn");
            }
	...
	public <T extends PrimaryKey> T detail(DataRuntime runtime, int index, T meta, PrimaryKey query, DataSet<DataRow> set) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends PrimaryKey> T detail(DataRuntime runtime, int index, T meta, PrimaryKey query, DataSet<DataRow> set)", 37));
		}
		return meta;
	}
	...
	public <T extends ForeignKey> LinkedHashMap<String, T> foreigns(DataRuntime runtime, String random, boolean greedy, ForeignKey query) {
		...
		try {
			...
			List<Run> runs = buildSelectForeignsRun(runtime, greedy, query);
			if(null != runs) {
				int idx = 0;
				for(Run run:runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectForeignsRun(DataRuntime runtime, boolean greedy,  ForeignKey query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectForeignsRun(DataRuntime runtime, boolean greedy,  ForeignKey query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Index> List<T> indexes(DataRuntime runtime, String random, boolean greedy, Collection<? extends Table> tables, Index query) {
		...
		try {
			...
			List<Run> runs = buildSelectIndexesRun(runtime, greedy, tabs);
			if (null != runs) {
				int idx = 0;
				for (Run run: runs) {
					DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Index> List<T> indexes(DataRuntime runtime, String random, boolean greedy, Index query) {
		...
		List<Run> runs = buildSelectIndexesRun(runtime, greedy, query);
		if(null != runs) {
			int idx = 0;
			for(Run run:runs) {
				DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Index> LinkedHashMap<String, T> indexes(DataRuntime runtime, String random, Index query) {
		...
		List<Run> runs = buildSelectIndexesRun(runtime, false, query);
	...
	public List<Run> buildSelectIndexesRun(DataRuntime runtime, boolean greedy, Index query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectIndexesRun(DataRuntime runtime, boolean greedy,  Index query)", 37));
		}
		return new ArrayList<>();
	}
	/**
	...
	public List<Run> buildSelectIndexesRun(DataRuntime runtime, boolean greedy,  Collection<? extends Table> tables) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectIndexesRun(DataRuntime runtime, boolean greedy,  Collection<? extends Table> tables)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Constraint> List<T> constraints(DataRuntime runtime, String random, boolean greedy, Constraint query) {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectConstraintsRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Constraint> LinkedHashMap<String, T> constraints(DataRuntime runtime, String random, Constraint query) {
        ...
        try{
            ...
            // 根据系统表查询
            try{
                List<Run> runs = buildSelectConstraintsRun(runtime, false, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectConstraintsRun(DataRuntime runtime, boolean greedy, Constraint query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectConstraintsRun(DataRuntime runtime, boolean greedy, Constraint query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Trigger> LinkedHashMap<String, T> triggers(DataRuntime runtime, String random, boolean greedy, Trigger query) {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectTriggersRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectTriggersRun(DataRuntime runtime, boolean greedy, Trigger query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 buildSelectTriggersRun(DataRuntime runtime, boolean greedy, Trigger query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Procedure> List<T> procedures(DataRuntime runtime, String random, boolean greedy, Procedure query) {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectProceduresRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Procedure> LinkedHashMap<String, T> procedures(DataRuntime runtime, String random, Procedure query) {
        ...
        try{
            ...
            // 根据系统表查询
            try{
                List<Run> runs = buildSelectProceduresRun(runtime, false, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectProceduresRun(DataRuntime runtime, boolean greedy, Procedure query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectProceduresRun(DataRuntime runtime, boolean greedy, Procedure query)", 37));
		}
		return new ArrayList<>();
	}
    ...
    public List<Run> buildSelectParametersRun(DataRuntime runtime, Procedure procedure) throws Exception {
        List<Run> runs = new ArrayList<>();
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectParametersRun(DataRuntime runtime, Procedure procedure)", 37));
        }
        return runs;
    }
	...
	public <T extends Procedure> List<T> procedures(DataRuntime runtime, boolean create, List<T> previous, Procedure query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends Procedure> List<T> procedures(DataRuntime runtime, int index, boolean create, List<T> procedures, DataSet<DataRow> set)", 37));
		}
        return previous;
	}
	...
	public <T extends Procedure> LinkedHashMap<String, T> procedures(DataRuntime runtime, boolean create, LinkedHashMap<String, T> previous, Procedure query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 <T extends Procedure> LinkedHashMap<String, T> procedures(DataRuntime runtime, int index, boolean create, LinkedHashMap<String, T> procedures, DataSet<DataRow> set)", 37));
		}
        return previous;
	}
    /**
    ...
    public <T extends Procedure> T procedure(DataRuntime runtime, String random, boolean greedy, Procedure query) throws Exception {
        ...
        List<Run> runs = buildSelectParametersRun(runtime, procedure);
     ...
     */
    @Override
    public <T extends Parameter> LinkedHashMap<String, T> parameters(DataRuntime runtime, int index, boolean create, LinkedHashMap<String, T> previous, Procedure procedure, DataSet<DataRow> set) throws Exception {
        if(null == previous) {
            previous = new LinkedHashMap<>();
        }
        for(DataRow row:set) {
	...
	public List<String> ddl(DataRuntime runtime, String random, Procedure procedure) {
		...
		try {
			long fr = System.currentTimeMillis();
			List<Run> runs = buildSelectDdlRun(runtime, procedure);
			if (null != runs && !runs.isEmpty()) {
				//直接查询DDL
				int idx = 0;
				for (Run run : runs) {
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, Procedure procedure) throws Exception {
		List<Run> runs = new ArrayList<>();
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, Procedure procedure)", 37));
		}
		return runs;
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, Procedure procedure, List<String> ddls, DataSet<DataRow> set) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> List<String> ddl(DataRuntime runtime, int index, Procedure procedure, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Function> List<T> functions(DataRuntime runtime, String random, boolean greedy, Function query) {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectFunctionsRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Function> LinkedHashMap<String, T> functions(DataRuntime runtime, String random, Function query) {
        ...
        try{
            ...
            // 根据系统表查询
            try{
                List<Run> runs = buildSelectFunctionsRun(runtime, false, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectFunctionsRun(DataRuntime runtime, boolean greedy, Function query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectFunctionsRun(DataRuntime runtime, boolean greedy, Function query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, Function meta) throws Exception {
		List<Run> runs = new ArrayList<>();
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, Function meta)", 37));
		}
		return runs;
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, Function function, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, Function function, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public <T extends Sequence> List<T> sequences(DataRuntime runtime, String random, boolean greedy, Sequence query) {
		...
		try{
			...
			try{
				...
				List<Run> runs = buildSelectSequencesRun(runtime, greedy, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public <T extends Sequence> LinkedHashMap<String, T> sequences(DataRuntime runtime, String random, Sequence query) {
		...
		try{
			...
			// 根据系统表查询
			try{
				List<Run> runs = buildSelectSequencesRun(runtime, false, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectSequencesRun(DataRuntime runtime, boolean greedy, Sequence query) {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectSequencesRun(DataRuntime runtime, boolean greedy, Sequence query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public List<Run> buildSelectDdlRun(DataRuntime runtime, Sequence meta) throws Exception {
		List<Run> runs = new ArrayList<>();
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectDdlRun(DataRuntime runtime, Sequence meta)", 37));
		}
		return runs;
	}
	...
	public List<String> ddl(DataRuntime runtime, int index, Sequence sequence, List<String> ddls, DataSet<DataRow> set) {
		...
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<String> ddl(DataRuntime runtime, int index, Sequence sequence, List<String> ddls, DataSet<DataRow> set)", 37));
		}
		return ddls;
	}
	...
	public StringBuilder type(DataRuntime runtime, StringBuilder builder, Column meta, String type, int ignoreLength, int ignorePrecision, int ignoreScale, int maxLength, int maxPrecision, int maxScale, int supportTimeZone, int supportLocalTimeZone) {
        ...
        meta.supportTimeZone(supportTimeZone);
        meta.supportLocalTimeZone(supportLocalTimeZone);
	...
	public <T extends Role> List<T> roles(DataRuntime runtime, String random, boolean greedy, Role query) throws Exception {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectRolesRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectRolesRun(DataRuntime runtime, boolean greedy, Role query) throws Exception {
        if(log.isDebugEnabled()) {
            log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectRolesRun(DataRuntime runtime, boolean greedy, Role query)", 37));
        }
		return new ArrayList<>();
	}
	...
	public <T extends User> List<T> users(DataRuntime runtime, String random, boolean greedy, User query) throws Exception {
        ...
        try{
            ...
            try{
                ...
                List<Run> runs = buildSelectUsersRun(runtime, greedy, query);
                if(null != runs) {
                    int idx = 0;
                    for(Run run:runs) {
                        DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectUsersRun(DataRuntime runtime, boolean greedy, User query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectUsersRun(DataRuntime runtime, User query)", 37));
		}
		return new ArrayList<>();
	}
	...
	public <T extends Privilege> List<T> privileges(DataRuntime runtime, String random, boolean greedy, Privilege query) throws Exception {
		...
		try{
			...
			try{
				...
				List<Run> runs = buildSelectPrivilegesRun(runtime, greedy, query);
				if(null != runs) {
					int idx = 0;
					for(Run run:runs) {
						DataSet<DataRow> set = selectMetadata(runtime, random, run);
	...
	public List<Run> buildSelectPrivilegesRun(DataRuntime runtime, boolean regreedy, Privilege query) throws Exception {
		if(log.isDebugEnabled()) {
			log.debug(LogUtil.format("子类(" + this.getClass().getSimpleName() + ")未实现 List<Run> buildSelectPrivilegesRun(DataRuntime runtime, Privilege query)", 37));
		}
		return new ArrayList<>();
	}
