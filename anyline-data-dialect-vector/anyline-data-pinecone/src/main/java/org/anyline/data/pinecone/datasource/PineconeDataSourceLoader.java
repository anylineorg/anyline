...
@AnylineComponent("anyline.environment.data.datasource.loader.pinecone")
public class PineconeDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    public static final Log log = LogProxy.get(PineconeDataSourceLoader.class);

    private final PineconeDataSourceHolder holder = PineconeDataSourceHolder.instance();
    ...
    public List<String> load() {
        ...
        if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            try {
                client = ConfigTable.environment().getBean(Pinecone.class);
            } catch (Exception e) {
                runtime = null;
            }
            if(null != client) {
                try {
                    runtime = holder().create("pinecone.default", client, false);
                    loadDefault = false;
                } catch (Exception e) {
                    runtime = null;
                    log.error("加载Pinecone数据源 异常:", e);
                }
            }
