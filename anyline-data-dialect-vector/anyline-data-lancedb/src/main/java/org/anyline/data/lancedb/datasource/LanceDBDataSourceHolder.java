...
package org.anyline.data.lancedb.datasource;
...
import org.anyline.data.lancedb.runtime.LanceDBRuntimeHolder;
...
@AnylineComponent("anyline.environment.data.datasource.holder.lancedb")
public class LanceDBDataSourceHolder extends AbstractDataSourceHolder implements DataSourceHolder{
	private static final LanceDBDataSourceHolder instance = new LanceDBDataSourceHolder();
	public static LanceDBDataSourceHolder instance() {
		return instance;
	}
	public LanceDBDataSourceHolder() {
		DataSourceHolder.register(LanceNamespace.class, this);
	}
	...
	public String reg(String key, String prefix) {
		try {
			...
			if(adapter.contains("lancedb")) {
				Map<String, Object> map = new HashMap<>();
				return inject(key, prefix, map, true);
			}
		} catch (Exception e) {
			log.error("注册LanceDB数据源 异常:", e);
		}
		return null;
	}
	...
	public String inject(String key, String prefix, Map<String, Object> params, boolean override) throws Exception {
		...
		try {
			String apiKey = value(prefix, params, "apiKey,api-key,token", String.class, null);
			String database = value(prefix, params, "database,dbName,database-name", String.class, null);
			String endpoint = value(prefix, params, "endpoint,url,uri", String.class, null);
			String region = value(prefix, params, "region", String.class, "us-east-1");
			...
			if(BasicUtil.isNotEmpty(endpoint)) {
				builder.endpoint(endpoint);
			}
			if(BasicUtil.isNotEmpty(region)) {
				builder.region(region);
			}

			LanceNamespace client = builder.build();
			LanceDBRuntimeHolder.instance().reg(key, client);
		} catch (Exception e) {
			log.error("[注册数据源失败][type:lancedb][key:{}][msg:{}]", key, e.toString());
			return null;
		}
		return null;
	}
	...
	public boolean validate(DataRuntime runtime) {
		LanceNamespace client = (LanceNamespace) runtime.getProcessor();
		return validate(client);
	}

	public boolean validate(LanceNamespace client) {
		try{
	...
	public boolean exeValidate(LanceNamespace client) {
		return true;
	}
