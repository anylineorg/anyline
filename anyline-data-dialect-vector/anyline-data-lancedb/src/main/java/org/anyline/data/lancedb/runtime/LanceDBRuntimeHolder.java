...
package org.anyline.data.lancedb.runtime;
...
import org.anyline.data.lancedb.adapter.LanceDBAdapter;
...
public class LanceDBRuntimeHolder extends AbstractRuntimeHolder {
	/**
	 * 临时数据源
	 */

	private static Map<String, LanceNamespace> temporary = new HashMap<>();

	private static final LanceDBRuntimeHolder instance = new LanceDBRuntimeHolder();
	public LanceDBRuntimeHolder() {
	}
	public static LanceDBRuntimeHolder instance() {
		return instance;
	}

	/**
	 * 注册数据源 子类覆盖 生成简单的DataRuntime不注册到spring
	 * @param client 数据源, 如 LanceNamespace
	 * @param database 数据库, jdbc类型数据源不需要
	 * @param adapter 如果确认数据库类型可以提供如 new LanceDBAdapter(), 如果不提供则根据ds检测
	 * @return DataRuntime
	 * @throws Exception 异常 Exception
	 */
	public DataRuntime temporary(Object client, String database, DriverAdapter adapter) throws Exception {
		LanceDBRuntime runtime = new LanceDBRuntime();
		if(null == adapter) {
			adapter = ConfigTable.environment().getBean(LanceDBAdapter.class);
		}
		if(client instanceof LanceNamespace) {
			String key = "temporary_lancedb";
			temporary.remove(key);
			//创建新数据源
			runtime.setKey(key);
			runtime.setAdapter(adapter);
			runtime.setProcessor(client);
			temporary.put(key, (LanceNamespace)client);
			log.warn("[创建临时数据源][key:{}][type:{}]", key, client.getClass().getSimpleName());
		}else{
			throw new Exception("请提供LanceNamespace兼容类型");
		}
		return runtime;
	}
	public DataRuntime reg(String key, LanceNamespace client) {
		String datasource_key = DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + key;
		log.info("[注入数据源][type:lancedb][key:{}][bean:{}]", key, datasource_key);
	 ...
	 * @param client LanceNamespace
	 * @param adapter adapter 可以为空 第一次执行时补齐
	 */
	public LanceDBRuntime reg(String datasource, LanceNamespace client, DriverAdapter adapter) {
		log.debug("[create lancedb runtime][key:{}]", datasource);
		if(null == adapter) {
			adapter = ConfigTable.environment().getBean(LanceDBAdapter.class);
		}
		LanceDBRuntime runtime = new LanceDBRuntime(datasource, client, adapter);
	...
	public boolean destroy(String key) {
		...
		if(null != monitor) {
			LanceDBRuntime runtime = (LanceDBRuntime) runtimes.get(key);
			if(null != runtime) {
