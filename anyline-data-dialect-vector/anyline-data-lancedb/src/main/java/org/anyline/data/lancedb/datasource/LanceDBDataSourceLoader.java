...
@AnylineComponent("anyline.environment.data.datasource.loader.lancedb")
public class LanceDBDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
	public static final Log log = LogProxy.get(LanceDBDataSourceLoader.class);

	private final LanceDBDataSourceHolder holder = LanceDBDataSourceHolder.instance();
	...
	public List<String> load() {
		...
		if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
			...
			LanceNamespace client = null;
			try{
				client = ConfigTable.environment().getBean(LanceNamespace.class);
			}catch (Exception e) {
				runtime = null;
			}
			if(null != client) {
				try {
					runtime =  holder().create("lancedb.default", client, false);
					loadDefault = false;
				}catch (Exception e) {
					runtime = null;
					log.error("加载LanceDB数据源 异常:", e);
				}
			}

			//有不支持通过connection返回获取连接信息的驱动，所以从配置文件中获取
			if(null != runtime) {
				String url = ConfigTable.environment().string( "spring.datasource.,anyline.datasource.", "url,uri,endpoint");
				runtime.setUrl(url);
