...
import org.anyline.data.lancedb.runtime.LanceDBRuntime;
...
@AnylineComponent("anyline.data.adapter.lancedb")
public class LanceDBAdapter extends AbstractDriverAdapter {

    public LanceDBAdapter() {
        super();
        for (LanceDBTypeMetadataAlias alias : LanceDBTypeMetadataAlias.values()) {
            clear(alias);
        }
        for (LanceDBTypeMetadataAlias alias: LanceDBTypeMetadataAlias.values()) {
            reg(alias);
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        return new LanceDBRun(runtime, dest);
    }
    ...
    public List<Run> buildDeleteRun(DataRuntime runtime, Table dest, ConfigStore configs, Object obj, Boolean placeholder, Boolean unicode, String ... columns) {
        ...
        if(obj instanceof ConfigStore) {
            LanceDBRun run = new LanceDBRun(runtime, dest);
    ...
    public List<Run> buildDeleteRunFromEntity(DataRuntime runtime, Table table, ConfigStore configs, Object obj, Boolean placeholder, Boolean unicode, String... columns) {
        List<Run> runs = new ArrayList<>();
        LanceDBRun run = new LanceDBRun(runtime, table);
        RunPrepare prepare = new DefaultTablePrepare();
        ...
        if(obj instanceof DataRow) {
            ...
            run.addRowCondition(row);
        }else{
            run.addEntityCondition(obj);
        }
        run.addCondition(columns);
        run.init();
        fillDeleteRunContent(runtime, run, placeholder, unicode);
    ...
    public void fillDeleteRunContent(DataRuntime runtime, Run run, Boolean placeholder, Boolean unicode) {
        if(run instanceof LanceDBRun) {
            LanceDBRun lr = (LanceDBRun) run;
            ConditionChain chain = lr.getConditionChain();
            String expression = buildExpression(chain);
            lr.setFilter(expression);
        }
    }
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Database meta) throws Exception {
        List<Run> runs = new ArrayList<>();
        LanceDBRun run = new LanceDBRun(runtime);
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Table meta) throws Exception {
        List<Run> runs = new ArrayList<>();
        LanceDBRun run = new LanceDBRun(runtime, meta);
    ...
    /**
     * ConditionChain → LanceDB filter expression
     * LanceDB filter syntax: field = value, field in (v1, v2), expr1 AND expr2
     */
    ...
    private String buildExpressionChain(ConditionChain chain) {
        ...
        String connector = (Condition.JOIN.OR == join) ? " OR " : " AND ";
    ...
    private String buildExpressionAuto(AutoCondition auto) {
        ...
        if(compare == Compare.IN) {
            return column + " IN " + formatLanceDBValues(values, true);
        }else if(compare == Compare.NOT_IN) {
            return column + " NOT IN " + formatLanceDBValues(values, true);
        }else if(compare == Compare.LIKE || compare == Compare.LIKE_PREFIX || compare == Compare.LIKE_SUFFIX) {
            return column + " LIKE " + formatLanceDBValue(values.get(0));
            ...
            if(values.size() >= 2) {
                return "(" + column + " >= " + formatLanceDBValue(values.get(0)) + " AND " + column + " <= " + formatLanceDBValue(values.get(1)) + ")";
            }
            return null;
        }else {
            String op = getLanceDBOperator(compare);
            return column + " " + op + " " + formatLanceDBValue(values.get(0));
        }
    }

    private String getLanceDBOperator(Compare compare) {
    ...
    private String formatLanceDBValue(Object value) {
    ...
    private String formatLanceDBValues(List<Object> values, boolean brackets) {
        StringBuilder builder = new StringBuilder();
        if(brackets) {
            builder.append("(");
        }
        ...
        for(Object value : values) {
            ...
            builder.append(formatLanceDBValue(value));
            first = false;
        }
        if(brackets) {
            builder.append(")");
