...
package org.anyline.data.faiss.adapter;

import org.anyline.annotation.AnylineComponent;
import org.anyline.data.adapter.init.AbstractDriverAdapter;
import org.anyline.data.faiss.run.FAissRun;
...
@AnylineComponent("anyline.data.adapter.faiss")
public class FAissAdapter extends AbstractDriverAdapter {
    ...
    public FAissActuator actuator() {
        return (FAissActuator) actuator;
    }
    ...
    public Run buildSelectRun(DataRuntime runtime, RunPrepare prepare, ConfigStore configs, Boolean placeholder, Boolean unicode, String ... conditions) {
        FAissRun run = new FAissRun(runtime, prepare.getTableName());
    ...
    /**
     * select [命令执行]<br/>
     * Faiss非关系型数据库，不使用SQL文本命令，直接通过SDK调用
     */
    @Override
    public DataSet<DataRow> selects(DataRuntime runtime, String random, boolean system, Table table, ConfigStore configs, Run run) {
        ...
        try{
            if(run instanceof FAissRun) {
                FAissRun r = (FAissRun) run;
                if(ConfigTable.IS_LOG_SQL && log.isInfoEnabled()) {
                    log.info("{}[cmd:select][index:{}][filter:{}]", random, r.getTableName(), r.getFilter());
                }
            } else {
                if(ConfigTable.IS_LOG_SQL && log.isInfoEnabled()) {
