...
@AnylineComponent("anyline.environment.data.datasource.loader.Coze")
public class CozeDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    ...
    public List<String> load() {
        ...
        if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            try{
                client = ConfigTable.environment().getBean(CozeClient.class);
            }catch (Exception e) {
                runtime = null;
            }
            if(null != client) {
                try {
                    runtime =  holder().create("Coze", client, false);
                    loadDefault = false;
                }catch (Exception e) {
                    runtime = null;
                    log.error("加载Coze数据源 异常:", e);
                }
            }
