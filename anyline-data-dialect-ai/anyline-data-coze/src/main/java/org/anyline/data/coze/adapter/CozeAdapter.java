...
@AnylineComponent("anyline.data.jdbc.adapter.Coze")
public class CozeAdapter extends AbstractDriverAdapter implements DriverAdapter {
    private static final Log log = LogProxy.get(CozeAdapter.class);

    @Override
    public DatabaseType type() {
        return DatabaseType.Coze;
    }
    ...
    public CozeAdapter() {
        super();
        for (CozeTypeMetadataAlias alias : CozeTypeMetadataAlias.values()) {
            clear(alias);
        }
        for (CozeTypeMetadataAlias alias : CozeTypeMetadataAlias.values()) {
            reg(alias);
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        CozeRun run = new CozeRun(runtime, dest);
        return run;
    }
    ...
    public Run buildQueryRun(DataRuntime runtime, RunPrepare prepare, ConfigStore configs,  Boolean placeholder, Boolean unicode, String ... conditions) {
        CozeRun run = new CozeRun(runtime, prepare.getTableName());
    ...
    public <T extends Column> LinkedHashMap<String, T> columns(DataRuntime runtime, String random, boolean greedy, Table table, Column query, boolean primary, ConfigStore configs) {
        ...
        CozeClient client = client(runtime);
        //
        return columns;
    }
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Table meta) throws Exception {
        List<Run> runs = new ArrayList<>();
        CozeRun run = new CozeRun(runtime, meta);
        run.setTable(meta);
    ...
    protected CozeClient client(DataRuntime runtime) {
        Object processor = runtime.getProcessor();
        return (CozeClient) processor;
    }
