...
@AnylineComponent("anyline.environment.data.datasource.loader.dify")
public class DifyDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    public static final Log log = LogProxy.get(DifyDataSourceLoader.class);

    private final DifyDataSourceHolder holder = DifyDataSourceHolder.instance();
    ...
    public List<String> load() {
        ...
        if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            DifyClient client = null;
            try{
                client = ConfigTable.environment().getBean(DifyClient.class);
            }catch (Exception e) {
                runtime = null;
            }
            if(null != client) {
                try {
                    runtime =  holder().create("dify", client, false);
                    loadDefault = false;
                }catch (Exception e) {
                    runtime = null;
                    log.error("加载Dify数据源 异常:", e);
                }
            }
