...
import org.anyline.data.dify.runtime.DifyRuntimeHolder;
...
@AnylineComponent("anyline.environment.data.datasource.holder.dify")
public class DifyDataSourceHolder extends AbstractDataSourceHolder implements DataSourceHolder{
	private static final DifyDataSourceHolder instance = new DifyDataSourceHolder();
	public static DifyDataSourceHolder instance() {
		return instance;
	}
	public DifyDataSourceHolder() {
		DataSourceHolder.register(DatabaseType.Dify, this);
	}
	...
	public String reg(String key, String prefix) {
		try {
			...
			if(adapter.contains("dify")) {
				Map<String, Object> map = new HashMap<>();
				return inject(key, prefix, map, true);
			}
		} catch (Exception e) {
			log.error("注册dify数据源 异常:", e);
		}
		return null;
	}
	...
	public boolean validate(DataRuntime runtime) {
		DifyClient client = (DifyClient) runtime.getProcessor();
		return validate(client);
	}

	public boolean validate(DifyClient client) {
		try{
