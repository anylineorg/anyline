...
public <T extends Table> LinkedHashMap<String, T> tables(DataRuntime runtime, String random, Table query, int types, int struct, ConfigStore configs) {
    ...
    public MetadataFieldRefer buildTableFieldRefer() {
        ...
        refer.setRefer("Catalog", "DB_NAME");
        refer.setRefer("Comment", "TABLE_COMMENT");
        return refer;
    }
    /**
     * table[命令合成]<br/>
    ...
    public MetadataFieldRefer buildMasterTableFieldRefer() {
        ...
        refer.setRefer("name", "STABLE_NAME");
        refer.setRefer("Catalog", "DB_NAME");
        refer.setRefer("Comment", "TABLE_COMMENT");
        return refer;
    }
    ...
    public <T extends PartitionTable> LinkedHashMap<String, T> partitions(DataRuntime runtime, int total, int index, boolean create, LinkedHashMap<String, T> previous, PartitionTable query, DataSet set) throws Exception {
        ...
        if(index == 0) {
            ...
            for(DataRow row:set) {
                ...
                previous.put(name, table); //不要转大写 下一步需要交集
            }
        }else if(index < total -1) {
            //与此之前的集合求交集
            previous.keySet().retainAll(set.getStrings("TABLE_NAME"));
        }
        if(index == total -1) {
            //最后一步 补充详细信息
            ...
            for(String key:result.keySet()) {
                previous.put(key.toUpperCase(), result.get(key));
            }
        }
        return previous;
    }
    ...
    public <T extends Tag> LinkedHashMap<String, T> tags(DataRuntime runtime, int index, boolean create, Table table, LinkedHashMap<String, T> previous, DataSet set) throws Exception {
        ...
        if(index ==0) {
            ...
            for(DataRow row:set) {
                ...
                T item = previous.get(name.toUpperCase());
                if(null == item) {
                    if(create) {
                        item = (T)new Tag();
                    }else{
                ...
                previous.put(name.toUpperCase(), item);
            }
        }else if(index ==1) {
            // DESCRIBE
            for(DataRow row:set) {
	...
	/**
	 * column[命令合成-子流程]<br/>
	 * 列定义:递增列,需要通过serial实现递增的在type(DataRuntime runtime, StringBuilder builder, Column meta)中实现,不支持
	 * @param runtime 运行环境主要包含驱动适配器 数据源或客户端
	 * @param builder builder
	 * @param meta 列
	 * @return StringBuilder
