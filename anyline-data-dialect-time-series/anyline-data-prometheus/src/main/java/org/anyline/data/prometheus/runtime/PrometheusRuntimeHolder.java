...
package org.anyline.data.prometheus.runtime;
...
import org.anyline.data.prometheus.adapter.PrometheusAdapter;
...
public class PrometheusRuntimeHolder extends AbstractRuntimeHolder {

    private static Map<String, Object> temporary = new HashMap<>();

    private static final PrometheusRuntimeHolder instance = new PrometheusRuntimeHolder();
    public PrometheusRuntimeHolder() {
    }
    public static PrometheusRuntimeHolder instance() {
        return instance;
    }

    public DataRuntime temporary(Object client, String database, DriverAdapter adapter) throws Exception {
        PrometheusRuntime runtime = new PrometheusRuntime();
        if(null == adapter) {
            adapter = ConfigTable.environment().getBean(PrometheusAdapter.class);
        }
        String key = "temporary_prometheus";
    ...
    public DataRuntime reg(String key, Object client) {
        ...
        log.info("[注入数据源][type:prometheus][key:{}][bean:{}]", key, datasource_key);
        ConfigTable.environment().regBean(datasource_key, client);
        return reg(key, client, null);
    }

    public PrometheusRuntime reg(String datasource, Object client, DriverAdapter adapter) {
        log.debug("[create prometheus runtime][key:{}]", datasource);
        if(null == adapter) {
            adapter = ConfigTable.environment().getBean(PrometheusAdapter.class);
        }
        PrometheusRuntime runtime = new PrometheusRuntime(datasource, client, adapter);
    ...
    public boolean destroy(String key) {
        ...
        if(null != monitor) {
            PrometheusRuntime runtime = (PrometheusRuntime) runtimes.get(key);
            if(null != runtime) {
