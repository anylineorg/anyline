...
public abstract class PostgresGenusAdapter extends AbstractJDBCAdapter {
    ...
    public List<Run> buildSelectRolesRun(DataRuntime runtime) throws Exception {
        ...
        runs.add(run);
        StringBuilder builder = run.getBuilder();
        builder.append("SELECT current_role as rolname");
        return runs;
    }
    /* *****************************************************************************************************************
     *
     ...
     * @return String
     */
    protected <T extends Table<T>> List<Run> buildSelectTablesRunWithPartBound(DataRuntime runtime, boolean greedy, Table<T> query, int types, ConfigStore configs) throws Exception {
        List<Run> runs = new ArrayList<>();
        Run run = new SimpleRun(runtime, configs);
        runs.add(run);
        StringBuilder builder = run.getBuilder();
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Schema meta) throws Exception {
        ...
        if(BasicUtil.isNotEmpty(owner)){
            builder.append(" AUTHORIZATION ").append(owner);
        }
        return runs;
    }
    ...
    public MetadataFieldRefer initRoleFieldRefer() {
        ...
        refer.map(Role.FIELD_NAME, "ROLNAME");
        return refer;
    }
