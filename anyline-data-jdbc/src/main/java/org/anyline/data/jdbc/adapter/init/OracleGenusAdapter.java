...
public abstract class OracleGenusAdapter extends AbstractJDBCAdapter {
    ...
    protected Run buildSelectColumnsBody(DataRuntime runtime,  ConfigStore configs) {
        Run run = new SimpleRun(runtime, configs);
        StringBuilder builder = run.getBuilder();
    ...
    //批量插入select 部分
    protected String insertsSelect(DataRuntime runtime, Run run, Table dest, DataSet<DataRow> set, ConfigStore configs, LinkedHashMap<String, Column> columns, Map<String, Sequence> sequens, PrimaryGenerator generator, LinkedHashMap<String, Column> pks) {
        StringBuilder builder = new StringBuilder();
