...
public abstract class ImpalaGenusAdapter extends AbstractJDBCAdapter {

    public ImpalaGenusAdapter() {
        super();
    ...
    public Database database(DataRuntime runtime, int index, boolean create, Database meta, DataSet<DataRow> set) throws Exception {
        if(null == meta) {
            for (DataRow row : set) {
                meta = new Database();
                meta.setName(row.getString("DATABASE_NAME"));
                return meta;
            }
        }
        return meta;
    ...
    public List<Run> buildSelectColumnsRun(DataRuntime runtime,  boolean metadata, Column query, ConfigStore configs) throws Exception {
        ...
        configs.and(refer.map(Column.FIELD_TABLE), query.getName());
        return runs;
    }
    ...
    public MetadataFieldRefer initDataTypeFieldRefer() {
        ...
        refer.map(DataTypeDefine.FIELD_LENGTH, "COLUMN_SIZE");
        refer.map(DataTypeDefine.FIELD_SCALE, "DECIMAL_DIGITS");
        return refer;
    }
    ...
    public List<Run> buildSelectConstraintsRun(DataRuntime runtime, boolean greedy, Constraint query) {
        ...
        configs.and("TABLE_SCHEMA", query.getSchemaName());
        configs.and("TABLE_NAME", query.getName());
        return runs;
    }
    ...
    public MetadataFieldRefer initConstraintFieldRefer() {
        ...
        refer.map(Constraint.FIELD_SCHEMA, "TABLE_SCHEMA");
