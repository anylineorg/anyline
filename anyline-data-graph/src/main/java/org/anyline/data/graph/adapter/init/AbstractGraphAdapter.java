...
/*            if(null != queryInterceptor) {
    ...
    protected Run fillSelectContent(DataRuntime runtime, TableRun run,  Boolean placeholder, Boolean unicode) {
        StringBuilder builder = run.getBuilder();
        fillSelectContent(runtime, builder, run);
        //UNION
    ...
    protected Run fillSelectContent(DataRuntime runtime, StringBuilder builder, TableRun run) {
     ...
     * @throws Exception 异常
     */
    public <T extends Trigger> LinkedHashMap<String, T> triggers(DataRuntime runtime, int index, boolean create, Table table, LinkedHashMap<String, T> previous, Trigger query, DataSet<DataRow> set) throws Exception {
        return previous;
    }
	 ...
	 */
	@Override
	public List<String> ddl(DataRuntime runtime, int index, Sequence meta, List<String> ddls, DataSet<DataRow> set) {
		return super.ddl(runtime, index, meta, ddls, set);
	}
