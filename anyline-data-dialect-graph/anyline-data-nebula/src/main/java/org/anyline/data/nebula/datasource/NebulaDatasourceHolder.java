...
@Component("anyline.data.datasource.holder.nebula")
public class NebulaDatasourceHolder extends DatasourceHolder {
	private static Logger log = LoggerFactory.getLogger(NebulaDatasourceHolder.class);
	...
	public static SessionPool reg(String key, SessionPool client, boolean override) throws Exception {
		return init(key, client, override);
	}
	public static SessionPool reg(String key, SessionPool client) throws Exception {
	...
	private static String inject(String key, String prefix, Map<String, Object> params, Environment env, boolean override) throws Exception {
		...
		if(BasicUtil.isEmpty(user)){
			user = value(env, prefix, "user", String.class, null);
		}
		String password = value(params, "password", String.class, null);
		if(BasicUtil.isEmpty(password)){
			password = value(env, prefix, "password", String.class, null);
		}
		...
		try {
			...
			*/
			...
			if(null == min){
				min = value(env, prefix, "minPoolSize,minSessionSize", Integer.class, null);
			}
			if(null != min) {
			...
			if(null == max){
				max = value(env, prefix, "maxPoolSize,maxSessionSize", Integer.class, null);
			}
			if(null != max) {
			...
			if(null == wait){
				wait = value(env, prefix, "wait,waitTime", Integer.class, null);
			}
			if(null != wait) {
			...
			if(null == timeout){
				timeout = value(env, prefix, "timeout,connectionTimeout", Integer.class, null);
			}
			if(null != timeout) {
			...
			factory.registerSingleton(datasource_id, session);
	...
	private static SessionPool init(String key, SessionPool client, boolean override) throws Exception {
		if(null != client) {
			check(key, override);
	...
	public static boolean validate(SessionPool client){
		try{
	...
	public static boolean exeValidate(SessionPool client){
