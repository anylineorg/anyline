...
@AnylineComponent("anyline.data.adapter.neo4j")
public class Neo4jAdapter extends AbstractGraphAdapter implements DriverAdapter {
    @Override
    ...
    public Neo4jAdapter() {
        ...
        delimiterFr = "`";
        delimiterTo = "`";
        for(Neo4jTypeMetadataAlias alias:Neo4jTypeMetadataAlias.values()) {
            clear(alias);
        }
        for (Neo4jTypeMetadataAlias alias: Neo4jTypeMetadataAlias.values()) {
            reg(alias);
    ...
    public void fillInsertContent(DataRuntime runtime, Run run, Table dest, DataSet<DataRow> set, ConfigStore configs, Boolean placeholder, Boolean unicode, LinkedHashMap<String, Column> columns) {
        ...
        RETURN id(n) AS pk1, id(m) AS pk2
        */
        ...
        for(int i=0; i<dataSize; i++) {
            ...
            foot.append(" id(n").append(i).append(") AS pk").append(i);
        }
        builder.append(foot);
    }
    ...
    public void fillInsertContent(DataRuntime runtime, Run run, Table dest, Collection list, ConfigStore configs, Boolean placeholder, Boolean unicode, LinkedHashMap<String, Column> columns) {
        ...
        for(Object obj:list) {
            ...
            foot.append(" id(n").append(idx).append(") AS pk").append(idx);
            idx ++;
        }
        builder.append(foot);
    }
    ...
    private Neo4jRow neo4j(Object obj){
        Neo4jRow neo4jRow = null;
        if(null != obj) {
            if (obj instanceof Neo4jRow) {
                neo4jRow = (Neo4jRow) obj;
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        ...
        if(obj instanceof Edge){
           ...
           if(nodes.size() > 1) {
               ...
               builder.append("MATCH (a:").append(table).append(a.json(a.getPrimaryKeys())).append(")");
               table = b.getTableName();
               if(BasicUtil.isEmpty(table) && null != dest){
                   table = dest.getName();
               }
        ...
        Boolean override = configs.override();
        if(null != override){
            Neo4jRow data = neo4j(obj);
            builder.append("MERGE (n:").append(dest.getName()).append(data.json(data.getPrimaryKey())).append(")\n");
            builder.append("ON CREATE SET ").append(data.setValue("n", data.keys()));
            if(override){
                builder.append("\nON MATCH SET ").append(data.setValue("n", data.keys()));
            }
            builder.append("\nRETURN id(n) AS pk");
/*
                MERGE (n:User{age: 20})
            ...
            builder.append("CREATE (n:").append(dest.getName()).append(" ").append(json(obj)).append(")").append(" RETURN id(n) AS pk");
        }
        return run;
    }
    ...
    public String deleteHead(DataRuntime runtime, ConfigStore configs, Table table, Object value) {
        StringBuilder builder = new StringBuilder();
        Neo4jRow neo = neo4j(value);
        if(null == neo && null != configs){
            neo = new Neo4jRow();
            ConfigChain chain = configs.getConfigChain();
            List<Config> list = chain.getConfigs();
            for(Config c : list){
                neo.put(c.getVariable(), c.getValue());
            }
    ...
    public List<Run> buildTruncateRun(DataRuntime runtime, Table table) {
        ...
        runs.add(run);
        StringBuilder builder = run.getBuilder();
        builder.append("MATCH (n:").append(table.getName()).append(") DETACH DELETE n");
        return runs;
    }
    ...
    public <T extends Table<T>> List<T> tables(DataRuntime runtime, String random, boolean greedy, Table<T> query, int types, int struct, ConfigStore configs) {
        ...
        List<VertexTable> vertexes = vertexes(runtime, random, greedy, vertex, types, struct, configs);
        for(VertexTable table:vertexes) {
            tables.add((T)table);
        }
    ...
    public <T extends Table<T>> LinkedHashMap<String, T> tables(DataRuntime runtime, String random, Table<T> query, int types, int struct, ConfigStore configs) {
        ...
        LinkedHashMap<String, VertexTable> vertexes = vertexes(runtime, random, vertex, types, struct, configs);
        for(VertexTable table:vertexes.values()) {
     ...
     */
    @Override
    public <T extends Table<T>> List<T> tables(DataRuntime runtime, int index, boolean create, List<T> tables, Table<T> query, DataSet<DataRow> set) throws Exception {
        if(null == tables) {
     ...
     */
    @Override
    public <T extends VertexTable<T>> LinkedHashMap<String, T> vertexes(DataRuntime runtime, int index, boolean create, LinkedHashMap<String, T> tables, VertexTable<T> query, DataSet<DataRow> set) throws Exception {
        return super.vertexes(runtime, index, create, tables, query, set);
    }
     ...
     */
    @Override
    public <T extends VertexTable<T>> List<T> vertexes(DataRuntime runtime, int index, boolean create, List<T> tables, VertexTable<T> query, DataSet<DataRow> set) throws Exception {
        return super.vertexes(runtime, index, create, tables, query, set);
    }
     ...
     */
    @Override
    public List<Run> buildSelectDdlRun(DataRuntime runtime, VertexTable meta) throws Exception {
     ...
     * [命令合成]
     * List<Run> buildSelectDdlRun(DataRuntime runtime, EdgeTable edge)
     * [结果集封装]<br/>
     ...
     */
    @Override
    public <T extends EdgeTable<T>> LinkedHashMap<String, T> edges(DataRuntime runtime, int index, boolean create, LinkedHashMap<String, T> tables, EdgeTable<T> query, DataSet<DataRow> set) throws Exception {
     ...
     */
    @Override
    public <T extends EdgeTable<T>> List<T> edges(DataRuntime runtime, int index, boolean create, List<T> tables, EdgeTable<T> query, DataSet<DataRow> set) throws Exception {
        return super.edges(runtime, index, create, tables, query, set);
    }
     ...
     * [命令合成]
     * List<Run> buildSelectViewsRun(DataRuntime runtime, boolean greedy, Catalog catalog, Schema schema, String pattern, int types)
     * [结果集封装]<br/>
     ...
     * [命令合成]
     * List<Run> buildSelectDdlRun(DataRuntime runtime, MasterTable table)
     * [结果集封装]<br/>
    ...
    public List<Run> buildSelectIndexesRun(DataRuntime runtime, boolean greedy,  Collection<? extends Table> tables) {
        ...
        for(Table table:tables) {
            ...
            runs.addAll(buildSelectIndexesRun(runtime, greedy, query));
        }
        return runs;
    }
