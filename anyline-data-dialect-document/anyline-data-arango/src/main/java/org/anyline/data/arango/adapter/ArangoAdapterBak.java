...
public class ArangoAdapterBak extends AbstractDriverAdapter implements DriverAdapter {
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        ...
        if(null != generator) {
            ...
            if(null == pv) {
                pv = BeanUtil.getFieldValue(obj, "_id", true);
            }
            if(null == pv) {
    ...
    public Run buildUpdateRunFromEntity(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, LinkedHashMap<String, Column> columns) {
        ...
        for(String pk : primaryKeys) {
            if(!cols.containsKey(pk.toUpperCase())) {
    ...
    private String buildAQL(ArangoRun run) {
        ...
        if(null != orders && !orders.isEmpty()) {
            sb.append(" SORT ");
            boolean first = true;
            for(Order od : orders.gets().values()) {
                if(!first) {
                    sb.append(", ");
                }
                sb.append("doc.").append(od.getColumn());
    ...
    public <T> long deletes(DataRuntime runtime, String random, int batch, Table table, ConfigStore configs, String key, Collection<T> values) {
        ConfigStore newConfigs = new DefaultConfigStore();
        newConfigs.and(key, values);
        return delete(runtime, random, table, newConfigs);
    }
    ...
    public long truncate(DataRuntime runtime, String random, Table table) {
        ...
        ArangoRuntime rt = (ArangoRuntime) runtime;
        ArangoDatabase database = rt.getDatabase();
        ArangoCollection cons = database.collection(table.getName());
        cons.truncate();
        result = 1;
        long millis = System.currentTimeMillis() - fr;
        if(ConfigTable.IS_LOG_SQL_TIME && log.isInfoEnabled()) {
