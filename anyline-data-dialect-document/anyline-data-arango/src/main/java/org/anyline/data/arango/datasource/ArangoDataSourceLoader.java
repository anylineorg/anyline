...
public class ArangoDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    ...
    public List<String> load() {
        ...
        if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            ArangoDatabase datasource = null;
            DataRuntime runtime = null;

            try{
                datasource = ConfigTable.environment().getBean(ArangoDatabase.class);
            }catch (Exception e) {
                runtime = null;
            }
            if(null != datasource) {
                try {
                    runtime =  holder().create("arango.default", datasource, false);
                    loadDefault = false;
                }catch (Exception e) {
                    runtime = null;
                    log.error("加载arango数据源 异常:", e);
                }
            }
