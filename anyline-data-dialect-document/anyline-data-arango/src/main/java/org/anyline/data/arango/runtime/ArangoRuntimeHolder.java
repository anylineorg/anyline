...
import org.anyline.data.arango.adapter.ArangoAdapter;
...
@AnylineComponent("anyline.environment.data.runtime.holder.arango")
public class ArangoRuntimeHolder extends AbstractRuntimeHolder implements RuntimeHolder {
    private static final ArangoRuntimeHolder instance = new ArangoRuntimeHolder();
    public static ArangoRuntimeHolder instance() {
        return instance;
    }
    public ArangoRuntimeHolder() {
    }

    /**
     * 注册数据源 子类覆盖 生成简单的DataRuntime不注册到
     * @param datasource 数据源, ArangoDatabase, ArangoDB
     * @param database 数据库, Arango类型数据源不需要
    ...
    public DataRuntime temporary(Object datasource, String database, DriverAdapter adapter) throws Exception {
        ArangoRuntime runtime = new ArangoRuntime();
        if(datasource instanceof ArangoDatabase || datasource instanceof ArangoDB) {
            String key = "temporary_arango";
            //关闭上一个
            close(key);
            ...
            throw new Exception("请提供 com.arangodb.ArangoDatabase 或 com.arangodb.ArangoDB 兼容类型");
        }
    ...
    public DataRuntime reg(String key, ArangoDB client, ArangoDatabase database) {
        String datasource_key = DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + key;
        log.info("[注入数据源][type:arango][key:{}][bean:{}]", key, datasource_key);
     ...
     * @param database ArangoDatabase
     * @param adapter adapter 可以为空 第一次执行时补齐
     */
    public ArangoRuntime reg(String datasource,  ArangoDB client, ArangoDatabase database, ArangoAdapter adapter) {
        log.debug("[create Arango runtime][key:{}]", datasource);
        ArangoRuntime runtime = new ArangoRuntime(datasource, client, database, adapter);
    ...
    public static void closeConnection(Object datasource) throws Exception {
        if(datasource instanceof ArangoDB) {
            ((ArangoDB) datasource).shutdown();
            log.info("[arango client shutdown]");
        } else if(datasource instanceof ArangoDatabase) {
            // ArangoDatabase 本身不维护连接池, 连接由所属 ArangoDB 管理
            // 如果单独持有 database 实例, 应通过 runtime.client() 获取 ArangoDB 来 shutdown
            log.info("[arango database instance released][name:{}]", ((ArangoDatabase) datasource).name());
        }
    }
}