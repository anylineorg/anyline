...
@AnylineComponent("anyline.data.jdbc.adapter.duckdb")
public class DuckDBAdapter extends AbstractJDBCAdapter implements JDBCAdapter {
	...
	public DuckDBAdapter() {
		super();
		delimiterFr = "`";
		delimiterTo = "`";
		for (DuckDBTypeMetadataAlias alias : DuckDBTypeMetadataAlias.values()) {
			clear(alias);
		}
		for (DuckDBTypeMetadataAlias alias: DuckDBTypeMetadataAlias.values()) {
			reg(alias);
