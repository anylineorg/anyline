...
public class HiveAdapter extends AbstractJDBCAdapter implements JDBCAdapter {
	...
	public HiveAdapter() {
		...
		for(HiveReader reader: HiveReader.values()) {
...
//		configs.in("TABLE_TYPE", tps);
	 ...
	 */
	@Override
	public List<Run> buildSelectColumnsRun(DataRuntime runtime,  boolean metadata, Column query, ConfigStore configs) throws Exception { List<Run> runs = new ArrayList<>();
		Run run = new SimpleRun(runtime, configs);
		runs.add(run);
		StringBuilder builder = run.getBuilder();
	...
	public List<Run> buildRenameRun(DataRuntime runtime, Procedure meta) throws Exception {
		log.warn("Hive不支持 rename procedure("+meta.getName()+")");
