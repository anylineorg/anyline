...
public class IoTDBAdapter extends MySQLGenusAdapter implements JDBCAdapter {
	...
	public List<Run> buildSelectTablesRun(DataRuntime runtime, boolean greedy, Table query, int types, ConfigStore configs) throws Exception {
		//只有这几列  没有类型列 database|   table_name|    ttl(ms)|status|comment|
		return super.buildSelectTablesRun(runtime, greedy, query, 0, configs);
	}
	...
	public MetadataFieldRefer initColumnFieldRefer() {
		...
		refer.map(Column.FIELD_AUTO_INCREMENT_CHECK, "");
		refer.map(Column.FIELD_AUTO_INCREMENT_CHECK_VALUE, "");

		refer.map(Column.FIELD_ON_UPDATE_CHECK, "");
		refer.map(Column.FIELD_ON_UPDATE_CHECK_VALUE, "");

		refer.map(Column.FIELD_PRIMARY_CHECK,"");
