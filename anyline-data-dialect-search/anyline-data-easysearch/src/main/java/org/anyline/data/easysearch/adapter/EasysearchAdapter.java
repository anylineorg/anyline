...
package org.anyline.data.easysearch.adapter;

import org.anyline.adapter.KeyAdapter;
import org.anyline.annotation.AnylineComponent;
import org.anyline.data.adapter.DriverAdapter;
import org.anyline.data.adapter.init.AbstractDriverAdapter;
import org.anyline.data.easysearch.metadata.EasysearchBuilder;
import org.anyline.data.easysearch.metadata.EasysearchIndex;
import org.anyline.data.easysearch.param.EasysearchConfigStore;
import org.anyline.data.easysearch.param.EasysearchRequestBody;
import org.anyline.data.easysearch.run.EasysearchRun;
...
@AnylineComponent("anyline.data.adapter.easysearch")
public class EasysearchAdapter extends AbstractDriverAdapter implements DriverAdapter {
    private static final Log log = LogProxy.get(EasysearchAdapter.class);

    @Override
    public DatabaseType type() {
        return DatabaseType.Easysearch;
    }
    ...
    public EasysearchAdapter() {
        super();
        for (EasysearchTypeMetadataAlias alias : EasysearchTypeMetadataAlias.values()) {
            clear(alias);
        }
        for (EasysearchTypeMetadataAlias alias : EasysearchTypeMetadataAlias.values()) {
            reg(alias);
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        EasysearchRun run = new EasysearchRun(runtime, dest);
...
*/
    ...
    public Run buildSelectRun(DataRuntime runtime, RunPrepare prepare, ConfigStore configs,  Boolean placeholder, Boolean unicode, String ... conditions) {
        EasysearchRun run = new EasysearchRun(runtime, prepare.getTableName());
        run.setMethod("POST");
        String endpoint = null;
        ...
        if(configs instanceof EasysearchConfigStore){
            //如果指定了body
            EasysearchRequestBody rb = null;
            EasysearchConfigStore esc = (EasysearchConfigStore)configs;
            ...
            if(BasicUtil.isEmpty(sql) && null != rp){
                Run sql_run = super.buildSelectRun(runtime, rp, new DefaultConfigStore(), placeholder, unicode);
                sql = sql_run.getFinalSelect(true);
        ...
        if(prepare instanceof TextPrepare){
            //service.selects("select * from ... ");
            Run tr = super.buildSelectRun(runtime, prepare, configs, placeholder, unicode, conditions);
            sql = tr.getFinalSelect(true);
        ...
        if(BasicUtil.isNotEmpty(sql)){
            if(null != navi){
                ...
                try {
                    String dsl = ((EasysearchActuator) actuator).dsl(this, runtime, null, sql);
    ...
    protected Run fillSelectContent(DataRuntime runtime, TableRun run,  Boolean placeholder, Boolean unicode) {
        EasysearchRun r = (EasysearchRun)run;
        ConditionChain chain = r.getConditionChain();

        ConfigStore configs = run.getConfigs();
        EasysearchRequestBody request = new EasysearchRequestBody();
        DataRow body = new OriginRow();
        if(null != configs) {
            ...
            if(configs instanceof EasysearchConfigStore){
                EasysearchConfigStore ecf = ((EasysearchConfigStore)configs);
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, boolean system, Table table, ConfigStore configs, Run run) {
        return selects(runtime, random, system, table, configs, (EasysearchRun) run);
    }
     ...
     * Catalog catalog(DataRuntime runtime, int index, boolean create, DataSet<DataRow> set)
     ******************************************************************************************************************/
    /**
     * catalog[调用入口]<br/>
     * @param runtime 运行环境主要包含驱动适配器 数据源或客户端
    ...
    public <T extends Table<T>> List<Run> buildSelectTablesRun(DataRuntime runtime, boolean greedy, Table<T> query, int types, ConfigStore configs) throws Exception {
        List<Run> runs = new ArrayList<>();
        EasysearchRun run = new EasysearchRun(runtime);
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Table meta) throws Exception {
        List<Run> runs = new ArrayList<>();
        EasysearchRun run = new EasysearchRun(runtime, meta);
        run.setTable(meta);
        ...
        LinkedHashMap<String, Object> map = EasysearchBuilder.build(meta);
    ...
    public LinkedHashMap<String,DataRow> analyze(DataRuntime runtime, String key, String mode) {
        ...
        if(response.getStatus() == 200) {
            ...
            DataSet<DataRow> tokens = row.getSet("tokens");
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, boolean system, Table table, ConfigStore configs, EasysearchRun run) {
        if(null == configs) {
            configs = new DefaultConfigStore();
        }
        configs.add(run);
