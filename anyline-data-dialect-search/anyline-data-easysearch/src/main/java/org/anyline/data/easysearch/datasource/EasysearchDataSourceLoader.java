...
package org.anyline.data.easysearch.datasource;
...
@AnylineComponent("anyline.environment.data.datasource.loader.easysearch")
public class EasysearchDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    public static final Log log = LogProxy.get(EasysearchDataSourceLoader.class);

    private final EasysearchDataSourceHolder holder = EasysearchDataSourceHolder.instance();
    ...
    public List<String> load() {
        ...
        if(!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            try{
                client = ConfigTable.environment().getBean(RestClient.class);
            }catch (Exception e) {
                runtime = null;
            }
            if(null != client) {
                try {
                    runtime =  holder().create("es.default", client, false);
                    loadDefault = false;
                }catch (Exception e) {
                    runtime = null;
                    log.error("加载Easysearch数据源 异常:", e);
                }
            }
