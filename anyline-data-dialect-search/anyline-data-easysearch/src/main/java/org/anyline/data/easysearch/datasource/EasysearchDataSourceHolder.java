...
import org.anyline.data.easysearch.runtime.EasysearchRuntimeHolder;
...
@AnylineComponent("anyline.environment.data.datasource.holder.easysearch")
public class EasysearchDataSourceHolder extends AbstractDataSourceHolder implements DataSourceHolder{
	private static final EasysearchDataSourceHolder instance = new EasysearchDataSourceHolder();
	public static EasysearchDataSourceHolder instance() {
		return instance;
	}
	public EasysearchDataSourceHolder() {
		DataSourceHolder.register(RestClient.class, this);
		DataSourceHolder.register(DatabaseType.Easysearch, this);
	}
	...
	public String reg(String key, String prefix) {
		try {
			...
			if(adapter.contains("easysearch") || adapter.contains("es")) {
				Map<String, Object> map = new HashMap<>();
				return inject(key, prefix, map, true);
			}
		} catch (Exception e) {
			log.error("注册Easysearch数据源 异常:", e);
		}
		return null;
	}
	...
	public String inject(String key, String prefix, Map<String, Object> params, boolean override) throws Exception {
		...
		try {
			...
			RestClient client = RestClient.builder(posts)
					.setRequestConfigCallback(requestConfigBuilder -> {
						...
						public void onFailure(Node node) {
							log.error("[ EasysearchClient ] >>  node :{}, host:{}, fail !", node.getName(),
									node.getHost());
						}
			...
			EasysearchRuntimeHolder.instance().reg(key, client);
		} catch (Exception e) {
			log.error("[注册数据源失败][type:Easysearch][key:{}][msg:{}]", key, e.toString());
			return null;
		}
