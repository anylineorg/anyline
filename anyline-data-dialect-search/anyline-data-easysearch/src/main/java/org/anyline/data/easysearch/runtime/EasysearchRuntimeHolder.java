...
package org.anyline.data.easysearch.runtime;
...
import org.anyline.data.easysearch.adapter.EasysearchAdapter;
...
public class EasysearchRuntimeHolder extends AbstractRuntimeHolder {
    /**
     * 临时数据源
     */
    ...
    private static final EasysearchRuntimeHolder instance = new EasysearchRuntimeHolder();
    public EasysearchRuntimeHolder() {
    }
    public static EasysearchRuntimeHolder instance() {
        return instance;
    }
    ...
    public DataRuntime temporary(Object client, String database, DriverAdapter adapter) throws Exception {
        EasysearchRuntime runtime = new EasysearchRuntime();
        if(null == adapter) {
            adapter = ConfigTable.environment().getBean(EasysearchAdapter.class);
        }
    ...
    public DataRuntime reg(String key, RestClient client) {
        ...
        log.info("[注入数据源][type:Easysearch][key:{}][bean:{}]", key, datasource_key);
     ...
     */
    public EasysearchRuntime reg(String datasource, RestClient client, DriverAdapter adapter) {
        log.debug("[create Easysearch runtime][key:{}]", datasource);
        if(null == adapter) {
            adapter = ConfigTable.environment().getBean(EasysearchAdapter.class);
        }
        EasysearchRuntime runtime = new EasysearchRuntime(datasource, client, adapter);
    ...
    public boolean destroy(String key) {
        ...
        if(null != monitor) {
            EasysearchRuntime runtime = (EasysearchRuntime) runtimes.get(key);
            if(null != runtime) {
                //这一步有可能抛出 异常
