...
public class Highlight {
    ...
    protected int boundary_max_scan		    = 20    ; // 扫描边界字符的距离。默认为20。
    ...
    protected boolean force_source          = false	; // 根据源高亮显示。默认值为false。
    ...
    protected int fragment_size			    = 100   ; // 字符中突出显示的片段的大小。默认为100。
    ...
    protected int no_match_size			    = 0     ; // 如果没有要突出显示的匹配片段，则希望从字段开头返回的文本量。默认为0(不返回任何内容)。
    protected int number_of_fragments       = 5	    ; // 返回的片段的最大数量。如果片段的数量设置为0，则不会返回任何片段。相反，突出显示并返回整个字段内容。当需要突出显示短文本(如标题或地址)，但不需要分段时，使用此配置非常方便。如果number_of_fragments为0，则忽略fragment_size。默认为5。
    ...
    protected int phrase_limit			    = 256   ; // 控制文档中所考虑的匹配短语的数量。防止fvh highlighter分析太多的短语和消耗太多的内存。提高限制会增加查询时间并消耗更多内存。默认为256。
    ...
    protected boolean require_field_match = true    ; // 默认情况下，只突出显示包含查询匹配的字段。将require_field_match设置为false以突出显示所有字段。默认值为true。
    ...
    public int getBoundary_max_scan() {
        return boundary_max_scan;
    }

    public void setBoundary_max_scan(int boundary_max_scan) {
    ...
    public void setFragment_size(int fragment_size) {
    ...
    public int getNo_match_size() {
        return no_match_size;
    }

    public void setNo_match_size(int no_match_size) {
        this.no_match_size = no_match_size;
    }

    public int getNumber_of_fragments() {
        return number_of_fragments;
    }

    public void setNumber_of_fragments(int number_of_fragments) {
    ...
    public int getPhrase_limit() {
        return phrase_limit;
    }

    public void setPhrase_limit(int phrase_limit) {
    ...
    public boolean isRequire_field_match() {
