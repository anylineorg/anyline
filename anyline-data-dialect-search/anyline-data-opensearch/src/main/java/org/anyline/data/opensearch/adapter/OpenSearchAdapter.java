...
import org.anyline.data.opensearch.metadata.OpenSearchBuilder;
import org.anyline.data.opensearch.metadata.OpenSearchIndex;
import org.anyline.data.opensearch.param.OpenSearchConfigStore;
import org.anyline.data.opensearch.param.OpenSearchRequestBody;
import org.anyline.data.opensearch.run.OpenSearchRun;
...
@AnylineComponent("anyline.data.opensearch.adapter")
public class OpenSearchAdapter extends org.anyline.data.adapter.init.AbstractDriverAdapter implements DriverAdapter {
    private static final Log log = LogProxy.get(OpenSearchAdapter.class);
    ...
    public OpenSearchAdapter() {
        super();
        for (OpenSearchTypeMetadataAlias alias : OpenSearchTypeMetadataAlias.values()) {
            clear(alias);
        }
        for (OpenSearchTypeMetadataAlias alias : OpenSearchTypeMetadataAlias.values()) {
            reg(alias);
    ...
    protected Run createInsertRun(DataRuntime runtime, Table dest, Object obj, ConfigStore configs, Boolean placeholder, Boolean unicode, List<String> columns) {
        OpenSearchRun run = new OpenSearchRun(runtime, dest);
...
*/
    ...
    public Run buildSelectRun(DataRuntime runtime, RunPrepare prepare, ConfigStore configs,  Boolean placeholder, Boolean unicode, String ... conditions) {
        OpenSearchRun run = new OpenSearchRun(runtime, prepare.getTableName());
        run.setMethod("POST");
        String endpoint = null;
        ...
        if(configs instanceof OpenSearchConfigStore){
            //如果指定了body
            OpenSearchRequestBody rb = null;
            OpenSearchConfigStore esc = (OpenSearchConfigStore)configs;
        ...
        if(BasicUtil.isNotEmpty(sql)){
            if(null != navi){
                ...
                try {
                    String dsl = ((OpenSearchActuator) actuator).dsl(this, runtime, null, sql);
    ...
    protected Run fillSelectContent(DataRuntime runtime, TableRun run,  Boolean placeholder, Boolean unicode) {
        OpenSearchRun r = (OpenSearchRun)run;
        ConditionChain chain = r.getConditionChain();

        ConfigStore configs = run.getConfigs();
        OpenSearchRequestBody request = new OpenSearchRequestBody();
        DataRow body = new OriginRow();
        if(null != configs) {
            ...
            if(configs instanceof OpenSearchConfigStore){
                OpenSearchConfigStore ecf = ((OpenSearchConfigStore)configs);
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, boolean system, Table table, ConfigStore configs, Run run) {
        return selects(runtime, random, system, table, configs, (OpenSearchRun) run);
    }
    ...
    public <T extends Table<T>> List<Run> buildSelectTablesRun(DataRuntime runtime, boolean greedy, Table<T> query, int types, ConfigStore configs) throws Exception {
        List<Run> runs = new ArrayList<>();
        OpenSearchRun run = new OpenSearchRun(runtime);
    ...
    public List<Run> buildCreateRun(DataRuntime runtime, Table meta) throws Exception {
        List<Run> runs = new ArrayList<>();
        OpenSearchRun run = new OpenSearchRun(runtime, meta);
        run.setTable(meta);
        ...
        LinkedHashMap<String, Object> map = OpenSearchBuilder.build(meta);
    ...
    public DataSet<DataRow> selects(DataRuntime runtime, String random, boolean system, Table table, ConfigStore configs, OpenSearchRun run) {
        if(null == configs) {
            configs = new DefaultConfigStore();
        }
        configs.add(run);
