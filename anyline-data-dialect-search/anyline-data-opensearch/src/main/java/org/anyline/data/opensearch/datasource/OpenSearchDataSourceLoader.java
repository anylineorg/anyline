...
@AnylineComponent("anyline.data.opensearch.datasource.loader")
public class OpenSearchDataSourceLoader extends AbstractDataSourceLoader implements DataSourceLoader {
    public static final Log log = LogProxy.get(OpenSearchDataSourceLoader.class);

    private final OpenSearchDataSourceHolder holder = OpenSearchDataSourceHolder.instance();
    ...
    public List<String> load() {
        ...
        if (!ConfigTable.environment().containsBean(DataRuntime.ANYLINE_DATASOURCE_BEAN_PREFIX + ".default")) {
            ...
            if (null != client) {
                try {
                    runtime = holder().create("opensearch.default", client, false);
                    loadDefault = false;
                } catch (Exception e) {
                    runtime = null;
                    log.error("加载OpenSearch数据源 异常:", e);
                }
            }
