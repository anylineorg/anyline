...
public class DefaultEnvironmentWorker implements EnvironmentWorker {
    private static Logger log = LoggerFactory.getLogger(DefaultEnvironmentWorker.class);
    ...
    public static void loadBean() throws Exception{
        ...
        if ("jar".equals(type)) {
            ...
            loadBean(new File(ConfigTable.getClassPath(), "anyline-bean.properties"));
        }
        //3.加载jar文件同目录的config
        File file = new File(FileUtil.merge(ConfigTable.getClassPath(), "config/anyline-bean.properties"));
        if(file.exists()){
    ...
    public static void loadBean(File file){
        if(null != file && file.exists()){
            if(file.isDirectory()){
                List<File> files = FileUtil.getAllChildrenFile(file, "anyline-bean.properties", ".jar");
                for(File item:files){
                    loadBean(item);
                }
            }else{
    ...
    public static void loadBean(String config){
        ...
        for(String line:lines){
            ...
            if(kv.length == 2){
                try {
                    reg(kv[0], new DefaultBeanDefine(kv[1], false));
                }catch (Exception e){
                    e.printStackTrace();
                }
            }
    ...
    public static Object instance(String name, BeanDefine define) {
        Object bean = null;
        ...
        try {
            ...
            for (String key : values.keySet()) {
                ...
                if (value instanceof ValueReference) {
                    value = instance.getBean(((ValueReference) value).getName());
    ...
    public static boolean reg(String name, Object bean){
        Object type = bean;
        if(bean instanceof BeanDefine){
            BeanDefine define = (BeanDefine)bean;
    ...
    public String string(String prefixes, String keys){
        ...
        for(String p:ps){
            for(String key:kss){
                ...
                value = ConfigTable.getString(sKey);
                if(null != value){
                    return value;
                }
